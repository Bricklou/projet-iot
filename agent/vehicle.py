...
NEEDED_COLUMNS = ['generationtime', 'id', 'speed_speedvalue', 'referenceposition_latitude', 'referenceposition_longitude']
...
def get_data(source_file, line_skip = 0, columns = [], number_of_lines = -1):
    ...
    with open(source_file, mode='r', newline='') as file:
        ...
        for row in reader:
            ...
            if line <= line_skip:
                continue
            data.append(compute_row(row, columns))
    return data  
...
def send_row(row, mqtt_client):
    ...
    json_row = json.dumps(named_row)
    request = mqtt_client.publish(TOPIC, json_row, qos=1)
